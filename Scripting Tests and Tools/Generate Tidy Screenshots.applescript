...
on run
	...
	-- Document Window
	-----------------------------	
	set the_window_id to set_document_window_size(1152, 720)
	
	tell application "System Events"
		tell process "Balthisar Tidy"
			tell window named "Sample Dirty Document.html"
				tell table 1 of scroll area 1 of group 3 of splitter group 1 of splitter group 1
					set focused to true
					tell row 1 to set selected to true
				end tell
			end tell
		end tell
	end tell
	...
	tell application "Balthisar Tidy"
		...
		set the_window_id to id of (window named "Balthisar Tidy Preferences")
		...
		repeat with i from 1 to countOfPrefsWindowPanels
			...
			if (countOfPrefsWindowPanels < 4) then
...
on take_screenshot_window(the_window_id, a_file_path, options)
	...
	---------------------------------------------------------
	tell application "System Events" to set frontmost of process "Balthisar Tidy" to true
	delay 1
